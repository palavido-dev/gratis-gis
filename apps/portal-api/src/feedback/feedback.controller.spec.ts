...
describe('FeedbackController', () => {
  ...
  describe('rate limiting', () => {
    ...
    it('reads the client address from X-Forwarded-For, not the socket', async () => {
      // The socket address is always Caddy's, so limiting on it would
      // rate-limit the whole internet as one client.
      const { controller, service } = makeController();
      ...
      expect(service.isRateLimited).toHaveBeenCalledWith('198.51.100.9');
