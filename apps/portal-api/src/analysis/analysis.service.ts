...
export class AnalysisService {
  ...
  /**
   * Height above ground (canopy/structure height) from a point
   * cloud: surface model minus ground model on one shared grid,
   * colored into a green ramp raster. Same validation surface as
   * hillshade since the input is the same.
   */
  async createHeightmapJob(
    user: AuthUser,
    sourceItemId: string,
    params: { resolution: number },
  ) {
    ...
    if (source.type !== 'point_cloud') {
      throw new BadRequestException(
        'Height above ground runs on point cloud items.',
      );
    }
    const data: unknown = source.data;
    if (!isPointCloudData(data) || !data.storageKey) {
    ...
    const target = await this.items.create(user, {
      type: 'tile_layer',
      title: `Height above ground (${source.title})`,
      description: `How tall everything is above the bare ground, from "${source.title}" at ${resolution}m: trees, buildings, and anything else with height. Deeper green means taller. Generated by the analysis worker.`,
      tags: ['height', 'canopy', 'derived', 'analysis'],
      data: {
        version: 1,
        format: 'cog',
        kind: 'raster',
